...
export default function UserApp() {
  ...
  useEffect(() => {
    ...
    onValue(ref(db, 'settings'), snap => { if (snap.exists()) setContactDetails(snap.val()); });
  ...
  const handleAuthSubmit = async (e) => {
    ...
    if (authMode === 'signup') {
      if (!pwdRegex.test(authForm.password)) return alert("Password: Min 6 characters, must include both letters and numbers.");
      ...
      get(ref(db, 'users')).then(snap => {
        ...
        if (exists) {
          alert("Welcome back! This number is already registered. Please log in.");
          setAuthMode('login');
        } else {
  ...
  const handleWhatsAppCheckout = () => {
    ...
    push(ref(db, 'orders'), { date: new Date().toLocaleDateString(), time: new Date().toLocaleTimeString(), customerName: currentUser.name, customerPhone: currentUser.phone, items: cart, total: cartTotal, status: 'Pending' });
    ...
    <div className="min-h-screen bg-[#FAFAFA] font-sans flex flex-col selection:bg-orange-100">
        ...
        {currentView === 'mealpass' && (
          <div className="max-w-5xl mx-auto px-4 sm:px-6 py-10 sm:py-16 text-center animate-in fade-in">
            {myPass && daysRemaining > 0 ? (
              <div className="bg-green-50 border border-green-200 p-8 sm:p-12 rounded-[2.5rem] shadow-xl text-center max-w-lg mx-auto transform hover:scale-105 transition-transform duration-500">
              ...
              <>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 sm:gap-8 text-left justify-center">
                  {(mealPassPlans && mealPassPlans.length > 0) ? mealPassPlans.map(plan => (
                     <div key={plan.id} className="bg-white p-8 sm:p-10 rounded-[2.5rem] border border-zinc-100 shadow-lg hover:shadow-2xl hover:-translate-y-2 transition-all duration-500 flex flex-col relative overflow-hidden group">
                     ...
                     <>
                       ...
                       <div className="bg-gradient-to-br from-zinc-900 to-black text-white p-8 sm:p-10 rounded-[2.5rem] border border-zinc-800 shadow-2xl hover:shadow-orange-900/20 hover:-translate-y-2 transition-all duration-500 flex flex-col relative overflow-hidden ring-1 ring-white/10 hover:ring-orange-500/50">
                         ...
                         <button onClick={() => handleSubscribe('Heavy Diet Pass')} className="w-full py-4 bg-orange-600 text-white rounded-2xl font-bold hover:bg-orange-500 active:scale-95 transition-all duration-300 mt-auto shadow-lg shadow-orange-900/50 text-sm sm:text-base">Get Premium Pass</button>
                       </div>
                     </>
                  )}
                </div>
      ...
      {showAuthModal && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-zinc-900/60 backdrop-blur-sm transition-opacity duration-300">
          <div className="bg-white p-8 sm:p-10 rounded-[2.5rem] w-full max-w-sm relative animate-in zoom-in-95 duration-300 shadow-2xl">
            ...
            <h3 className="text-3xl font-extrabold mb-2 text-zinc-900">{authMode === 'login' ? 'Welcome Back!' : 'Create Account'}</h3>
            ...
            <form onSubmit={handleAuthSubmit} className="space-y-4">
              {authMode === 'signup' && (
                <input 
                  ...
                  required 
                  className="w-full bg-zinc-50 border border-zinc-200 p-4 rounded-2xl outline-none focus:border-orange-500 focus:bg-white transition-colors duration-300 font-medium text-zinc-900" 
              ...
              <input 
                ...
                className="w-full bg-zinc-50 border border-zinc-200 p-4 rounded-2xl outline-none focus:border-orange-500 focus:bg-white transition-colors duration-300 font-medium text-zinc-900 tracking-wider" 
                onChange={e => {
              ...
              <button type="submit" className="w-full bg-orange-600 text-white py-4 rounded-2xl font-bold text-lg hover:bg-orange-500 shadow-xl shadow-orange-600/20 active:scale-[0.98] transition-all duration-300 mt-4">
      ...
      {isCartOpen && (
        <div className="fixed inset-0 z-50 flex justify-end">
          <div className="absolute inset-0 bg-zinc-900/60 backdrop-blur-sm transition-opacity duration-300" onClick={() => setIsCartOpen(false)}></div>
          <div className="relative w-full sm:max-w-md bg-white h-full p-6 sm:p-8 flex flex-col shadow-2xl animate-in slide-in-from-right duration-500 ease-out">
             <div className="flex justify-between items-center mb-8">
               <h2 className="font-extrabold text-3xl text-zinc-900">Your Order</h2>
               <X className="cursor-pointer bg-zinc-100 p-2 rounded-full hover:bg-zinc-200 w-10 h-10 transition-colors duration-300 text-zinc-500" onClick={() => setIsCartOpen(false)}/>
             </div>
             <div className="flex-1 overflow-y-auto space-y-5 pr-2">
                {(cart || []).map(item => (
                  <div key={item.cartItemId} className="flex justify-between items-center border border-zinc-100 p-4 rounded-3xl shadow-sm bg-zinc-50/50 hover:bg-white hover:shadow-md transition-all duration-300">
                    <div>
                    ...
                    <div className="flex items-center gap-4">
                      <div className="font-extrabold text-xl text-zinc-900">₹{item.price * item.quantity}</div>
                      <button onClick={() => setCart(cart.filter(c => c.cartItemId !== item.cartItemId))} className="text-rose-500 bg-white border border-rose-100 p-2.5 rounded-xl hover:bg-rose-50 hover:border-rose-200 transition-all duration-300 shadow-sm"><Trash2 className="w-4 h-4"/></button>
                    </div>
                  </div>
                ))}
                {(!cart || cart.length === 0) && (
             ...
             {cart && cart.length > 0 && (
               <div className="pt-6 border-t border-zinc-100 mt-auto bg-white">
                 <div className="flex justify-between items-end mb-6">
                   ...
                   <span className="text-3xl font-extrabold text-zinc-900">₹{cartTotal}</span>
                 </div>
                 <button onClick={handleWhatsAppCheckout} className="w-full bg-green-600 text-white py-5 rounded-2xl font-bold text-lg hover:bg-green-500 shadow-xl shadow-green-900/20 active:scale-[0.98] transition-all duration-300 flex justify-center items-center gap-2">
