    ...
    <div className="min-h-screen bg-zinc-950 flex items-center justify-center p-4 font-sans text-zinc-300 selection:bg-orange-500/30">
      <div className="bg-zinc-900/40 backdrop-blur-2xl border border-zinc-800/50 p-8 sm:p-10 rounded-[2.5rem] max-w-sm w-full shadow-[0_0_50px_-12px_rgba(0,0,0,0.8)] animate-in zoom-in-95 duration-500 ease-out">
        ...
        <input 
          ...
          className="w-full bg-zinc-950/50 border border-zinc-800/80 p-4 rounded-2xl text-white mb-6 outline-none focus:border-orange-500 focus:bg-zinc-900 focus:ring-4 focus:ring-orange-500/10 transition-all duration-300 text-center tracking-[0.3em] font-black shadow-inner" 
          placeholder="••••••••" 
        />
        <button onClick={() => onLogin(pwd)} className="w-full bg-gradient-to-r from-orange-600 to-orange-500 text-white font-bold py-4 rounded-2xl hover:shadow-[0_8px_30px_-8px_rgba(249,115,22,0.5)] active:scale-95 transition-all duration-300">
          Unlock Dashboard
        </button>
      </div>
    </div>
...
export default function AdminPanel() {
  ...
  let totalIncome = 0; let totalExpense = 0; let modeTotals = { Online: 0, Cash: 0, Udhar: 0 };
  filteredFinance.forEach(f => {
    const amt = Number(f.amount) || 0;
    if (f.category === 'Expense') totalExpense += amt;
    else { totalIncome += amt; if(f.type) modeTotals[f.type] = (modeTotals[f.type] || 0) + amt; }
  });
  ...
  const downloadBulkTemplate = () => {
    const template = [{ name: 'Aloo Paratha', price: 60, category: 'Paratha', description: 'Fresh and hot', image: 'Drive Link here', allowAddons: true }];
        ...
        results.data.forEach(item => {
          if (item.name && item.price) {
            ...
            if (!existingCategoryNames.includes(catName.toLowerCase()) && !newlyAddedCategories.includes(catName.toLowerCase())) {
               ...
               newlyAddedCategories.push(catName.toLowerCase()); 
            }
          }
        });
    ...
    <div className="min-h-screen bg-zinc-950 text-zinc-300 p-2 sm:p-4 md:p-8 font-sans pb-24 overflow-x-hidden selection:bg-orange-500/30">
      <div className="max-w-7xl mx-auto">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 border-b border-zinc-800/50 pb-6 gap-4">
          ...
          <button onClick={() => setIsAdminAuthenticated(false)} className="bg-zinc-900/50 border border-zinc-800/80 px-5 py-2.5 rounded-xl text-sm font-semibold transition-all duration-300 hover:bg-zinc-800 hover:text-white flex items-center w-full sm:w-auto justify-center group shadow-sm hover:shadow-md"><LogOut className="inline w-4 h-4 mr-2 group-hover:-translate-x-1 transition-transform"/>Logout</button>
        </div>

        <div className="flex space-x-2 border-b border-zinc-800/50 mb-8 overflow-x-auto pb-4 scrollbar-hide w-full">
          {['Overview', 'Recent Orders', 'Discounts', 'Finance Tracker', 'Meal Passes', 'User Logins', 'Manage Categories', 'Manage Menu', 'Bulk Update', 'Settings'].map(t => (
            <button key={t} onClick={() => setTab(t.toLowerCase().replace(/ /g, ''))} className={`px-4 py-2.5 rounded-xl text-xs sm:text-sm font-semibold whitespace-nowrap transition-all duration-300 ${tab === t.toLowerCase().replace(/ /g, '') ? 'bg-orange-500/10 text-orange-500 border border-orange-500/30 shadow-inner' : 'text-zinc-500 hover:text-zinc-300 hover:bg-zinc-900/50'}`}>{t}</button>
          ))}
        </div>
        ...
        {tab === 'overview' && (
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-4 sm:gap-6 animate-in fade-in slide-in-from-bottom-4 duration-700 ease-out">
            <div className={`${cardStyle} p-6 border-t-4 border-t-purple-500 hover:-translate-y-1`}><div className="text-[10px] font-bold mb-1 uppercase text-zinc-500 tracking-wider">Visitors</div><div className="text-2xl sm:text-3xl font-extrabold text-white">{visitors}</div></div>
            <div className={`${cardStyle} p-6 border-t-4 border-t-orange-500 hover:-translate-y-1`}><div className="text-[10px] font-bold mb-1 uppercase text-zinc-500 tracking-wider">Total Orders</div><div className="text-2xl sm:text-3xl font-extrabold text-white">{orders.length}</div></div>
            <div className={`${cardStyle} p-6 border-t-4 border-t-blue-500 hover:-translate-y-1`}><div className="text-[10px] font-bold mb-1 uppercase text-zinc-500 tracking-wider">Users</div><div className="text-2xl sm:text-3xl font-extrabold text-white">{registeredUsers.length}</div></div>
            <div className={`${cardStyle} p-6 border-t-4 border-t-pink-500 hover:-translate-y-1`}><div className="text-[10px] font-bold mb-1 uppercase text-zinc-500 tracking-wider">Active Passes</div><div className="text-2xl sm:text-3xl font-extrabold text-white">{mealPassSubscribers.length}</div></div>
            <div className={`${cardStyle} p-6 border-t-4 border-t-green-500 col-span-2 sm:col-span-1 hover:-translate-y-1`}><div className="text-[10px] font-bold mb-1 uppercase text-zinc-500 tracking-wider">Revenue</div><div className="text-2xl sm:text-3xl font-extrabold text-white">₹{orders.reduce((sum, o) => sum + (o.total || 0), 0)}</div></div>
          </div>
        )}
        ...
        {tab === 'recentorders' && (
          <div className={`${cardStyle} overflow-hidden animate-in fade-in slide-in-from-bottom-4 duration-700 ease-out divide-y divide-zinc-800/50`}>
             {orders.map(o => {
               ...
               return (
                 <div key={o.id} className="p-5 sm:p-6 flex flex-col md:flex-row justify-between items-start md:items-center gap-4 hover:bg-zinc-800/30 transition-colors duration-300">
                   <div>
                     <div className="text-white font-bold text-base sm:text-lg mb-1">{o.customerName}</div>
                     <div className="text-xs text-zinc-500 font-medium font-mono bg-zinc-950/50 inline-block px-3 py-1 rounded-lg border border-zinc-800/50">{o.customerPhone} | {o.date} • {o.time}</div>
                     {o.promoCode && o.promoCode !== 'None' && <div className="mt-2 text-[10px] text-green-400 font-bold bg-green-500/10 px-2 py-0.5 rounded inline-block border border-green-500/20">COUPON: {o.promoCode} (-₹{o.discount})</div>}
                   </div>
                   ...
                   <div className="flex flex-col sm:items-end w-full md:w-auto gap-3">
                     ...
                     <div className="flex gap-2 w-full md:w-auto">
                       <select 
                         ...
                         onChange={(e) => update(ref(db, `orders/${o.id}`), { status: e.target.value })}
                         className={`outline-none appearance-none cursor-pointer px-4 py-2 rounded-xl text-xs font-bold uppercase tracking-widest border transition-all duration-300 ${statusColors[o.status || 'Pending']}`}
                       >
                       ...
                       <button onClick={() => remove(ref(db, `orders/${o.id}`))} className="text-zinc-600 hover:text-rose-500 bg-zinc-900 border border-zinc-800 p-2.5 rounded-xl transition-all duration-300"><Trash2 className="w-4 h-4"/></button>
                     </div>
                   </div>
                 </div>
               );
        ...
        {tab === 'discounts' && (
          <div className="space-y-6 max-w-5xl animate-in fade-in slide-in-from-bottom-4 duration-700 ease-out">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
              {promoCodes.map(p => (
                <div key={p.id} className={`${cardStyle} p-6 flex flex-col justify-between transition-all duration-500 ${p.isActive === false ? 'border-l-4 border-l-zinc-700 opacity-60 grayscale' : 'border-l-4 border-l-green-500 hover:-translate-y-1'}`}>
                  <div className="flex justify-between items-start mb-5">
                    <div>
                      ...
                      <div className={`text-sm font-bold mt-1 ${p.isActive === false ? 'text-zinc-500' : 'text-green-400'}`}>
                        {p.type === 'PERCENT' ? `${p.value}% OFF (Upto ₹${p.maxDiscount})` : `Flat ₹${p.value} OFF`}
                      </div>
                      <div className="text-[10px] text-zinc-500 uppercase tracking-widest mt-2 bg-zinc-950 inline-block px-2.5 py-1 rounded-md border border-zinc-800">Min Order: ₹{p.minOrder}</div>
                    </div>
                    <div className="flex gap-2">
                      <button onClick={() => handleEditPromo(p)} className="text-blue-400 hover:text-white bg-blue-500/10 hover:bg-blue-500 p-2.5 rounded-xl transition-all duration-300"><Edit className="w-4 h-4"/></button>
                  ...
                  >
                    <Power className="w-4 h-4"/> {p.isActive === false ? 'Turn ON Coupon' : 'Turn OFF Coupon'}
                  </button>
                </div>
              ))}
              {promoCodes.length === 0 && <div className="col-span-full text-center p-12 text-zinc-600 font-semibold text-sm">No Coupons Active.</div>}
            </div>
          </div>
        )}
        ...
        {tab === 'financetracker' && (
           <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-700 ease-out">
             <div className={`${cardStyle} p-5 sm:p-6 flex flex-col md:flex-row gap-4 items-start md:items-end`}>
                ...
                <button onClick={() => downloadCSV(filteredFinance, "finance_report")} className="w-full md:w-auto bg-zinc-800/80 border border-zinc-700 text-white px-6 py-3.5 rounded-xl font-bold flex items-center justify-center gap-2 hover:bg-zinc-700 transition-all duration-300 text-sm"><Download className="w-4 h-4"/> Export</button>
             </div>
             ...
             <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 sm:gap-6">
               ...
               <div className={`${cardStyle} p-6 border-b-4 border-b-blue-500 relative overflow-hidden`}>
                 ...
                 <div className="text-[10px] font-bold uppercase text-zinc-500 tracking-wider mb-2">Net Balance</div>
             ...
             >
               ...
               <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
                 ...
                 <select className={inputStyle} value={newFinance.type} onChange={e => setNewFinance({...newFinance, type: e.target.value})}>
                    <option value="Online">Online</option><option value="Cash">Cash</option><option value="Udhar">Udhar</option>
                 </select>
               ...
               <table className="w-full text-left text-sm min-w-[700px]">
                 ...
                 <tbody className="divide-y divide-zinc-800/30">
                   {filteredFinance.map(f => (
                     <tr key={f.id} className="hover:bg-zinc-800/40 transition-colors duration-300 group">
        ...
        {tab === 'mealpasses' && (
          <div className="space-y-10 animate-in fade-in slide-in-from-bottom-4 duration-700 ease-out">
                ...
                {mealPassSubscribers.map(s => (
                  <div key={s.id} className={`${cardStyle} p-6 flex justify-between items-center hover:-translate-y-1 transition-all duration-300 border-l-4 border-l-pink-500`}>
                    ...
                    <div className="flex flex-col gap-3 border-l border-zinc-800/80 pl-4">
                      <button onClick={() => { setEditPassId(s.id); setEditPassData({ name: s.name, phone: s.phone, plan: s.plan }); window.scrollTo({ top: 0, behavior: 'smooth' }); }} className="text-blue-400 hover:text-white bg-blue-500/10 hover:bg-blue-500 p-2.5 rounded-xl transition-all duration-300"><Edit className="w-4 h-4"/></button>
                      <button onClick={() => remove(ref(db, `meal_passes/${s.id}`))} className="text-rose-500 hover:text-white bg-rose-500/10 hover:bg-rose-500 p-2.5 rounded-xl transition-all duration-300"><Trash2 className="w-4 h-4"/></button>
                    </div>
                  </div>
                ))}
              </div>
            ...
            <div>
              <h3 className="text-white font-extrabold text-xl mb-6 border-b border-zinc-800/50 pb-3 flex items-center gap-2"><ShieldCheck className="text-orange-500"/> Manage Packages</h3>
              <form onSubmit={savePlan} className={`${cardStyle} p-6 sm:p-8 grid grid-cols-1 md:grid-cols-2 gap-5 mb-8`}>
                 ...
                 <button type="submit" className={`w-full md:col-span-2 mt-2 ${btnPrimary}`}>{editPlanId ? 'Update Package' : 'Create Package'}</button>
              </form>
            ...
            <table className="w-full text-left text-sm min-w-[500px]">
              ...
              <tbody className="divide-y divide-zinc-800/30">
                {registeredUsers.map(u => (
                  <tr key={u.id} className="hover:bg-zinc-800/40 transition-colors duration-300 group">
        ...
        {tab === 'managecategories' && (
          <div className="space-y-6 max-w-xl animate-in fade-in slide-in-from-bottom-4 duration-700 ease-out">
             <form onSubmit={(e) => { e.preventDefault(); if(!newCategoryName) return; push(ref(db, 'categories'), { name: newCategoryName }); setNewCategoryName(''); }} className="flex gap-3">
                ...
                <button type="submit" className="bg-gradient-to-r from-orange-600 to-orange-500 text-white px-6 rounded-xl font-bold hover:shadow-lg hover:shadow-orange-500/30 transition-all duration-300 active:scale-95"><PlusCircle className="w-5 h-5"/></button>
             </form>
             <div className="grid grid-cols-1 gap-3">
               {categories.map(c => (
                 <div key={c.id} className={`${cardStyle} p-4 px-5 flex justify-between items-center group`}>
        ...
        {tab === 'bulkupdate' && (
          <div className="max-w-2xl mx-auto py-10 text-center space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700 ease-out">
            <div className={`${cardStyle} p-10 sm:p-14 border-2 border-dashed border-zinc-700 hover:border-orange-500/50 flex flex-col items-center transition-all duration-500 group cursor-pointer relative overflow-hidden`}>
        ...
        {tab === 'settings' && (
          <div className={`max-w-2xl mx-auto ${cardStyle} p-8 sm:p-12 space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-700 ease-out`}>
             <div className="flex items-center gap-3 mb-8 border-b border-zinc-800/50 pb-6">
               ...
               <h3 className="text-white font-extrabold text-2xl tracking-tight">Store & Delivery Settings</h3>
             </div>
             ...
             <div><label className="text-[10px] uppercase font-bold text-zinc-500 mb-2 block tracking-widest">Email ID</label><input className={inputStyle} value={contactDetails?.email || ''} onChange={e => setContactDetails(prev => ({...prev, email: e.target.value}))} /></div>
             <div><label className="text-[10px] uppercase font-bold text-zinc-500 mb-2 block tracking-widest">Instagram Username</label><input className={inputStyle} value={contactDetails?.instagram || ''} onChange={e => setContactDetails(prev => ({...prev, instagram: e.target.value}))} /></div>
             <div><label className="text-[10px] uppercase font-bold text-zinc-500 mb-2 block tracking-widest">Full Address</label><input className={inputStyle} value={contactDetails?.address || ''} onChange={e => setContactDetails(prev => ({...prev, address: e.target.value}))} /></div>
